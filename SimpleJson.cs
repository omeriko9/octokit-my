 ...
 class ReflectionUtils
        {
            ...
            public static SetDelegate GetSetMethodByExpression(PropertyInfo propertyInfo)
            {
                ...
                SetDelegate tmp = delegate(object source, object val)
                {
                    try
                    {
                        int propVal = -1;
                        if (val != null && !String.IsNullOrEmpty(val.ToString()) && int.TryParse(val.ToString(), out propVal))
                            compiled(source, propVal);
                        else
                            compiled(source, val);
                    }
