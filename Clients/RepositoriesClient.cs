...
namespace Octokit
{
    ...
    public class RepositoriesClient : ApiClient, IRepositoriesClient
    {
        ...
        public Task<CommitWrapper> GetCommit(string owner, string name, string commitSha)
        {
            Ensure.ArgumentNotNullOrEmptyString(owner, "owner");
            Ensure.ArgumentNotNullOrEmptyString(name, "name");
            Ensure.ArgumentNotNullOrEmptyString(commitSha, "commitSha");

            var endpoint = ("repos/{0}/{1}/commits/" + commitSha).FormatUri(owner, name);
            return ApiConnection.Get<CommitWrapper>(endpoint);

        }

        public Task<ContentFile> GetContent(string owner, string name, string fullPath)
        {
            Ensure.ArgumentNotNullOrEmptyString(owner, "owner");
            Ensure.ArgumentNotNullOrEmptyString(name, "name");
            Ensure.ArgumentNotNullOrEmptyString(fullPath, "fullPath");

            var endpoint = ("repos/{0}/{1}/contents/" + fullPath).FormatUri(owner, name);
            return ApiConnection.Get<ContentFile>(endpoint);
        }
